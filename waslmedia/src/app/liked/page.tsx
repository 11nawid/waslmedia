...
import { getLikedVideos } from '@/server/services/videos';
...
export default async function LikedVideosPage() {
  ...
  const videos: Video[] = user ? await getLikedVideos(user.id) : [];

  if (!user) {
    return (
        <MainContent>
            <div className="flex flex-col items-center justify-center h-full text-center">
                <ThumbsUp className="w-24 h-24 text-muted-foreground mb-4" />
                <h2 className="text-2xl font-bold">Sign in to see your liked videos</h2>
                <p className="text-muted-foreground mt-2">Sign in to access videos that you've liked.</p>
  ...
  return (
    <MainContent>
         <div className="flex flex-col md:flex-row gap-8 p-4 md:p-8 bg-gradient-to-b from-blue-500/20 to-background min-h-full">
            <div className="w-full md:w-1/4 md:max-w-sm flex-shrink-0">
                <div className="sticky top-20">
                    <div className={`w-full rounded-lg overflow-hidden mb-4 relative bg-secondary ${firstVideo && isShortVideo(firstVideo) ? 'aspect-[9/16] max-w-[15rem]' : 'aspect-video'}`}>
                        {firstVideo ? (
                  <VideoThumbnail thumbnailUrl={firstVideo.thumbnailUrl} videoUrl={firstVideo.videoUrl} alt="Liked videos" sizes="384px" />
                        ) : (
                    ...
                    <div className="space-y-3">
                        {videos.map((video, index) => (
                            <div key={video.id} className="flex gap-4 items-center group">
                                ...
                                <VideoCard video={video} variant="list" sourceContext="liked" />
                            </div>
                        ))}
                    </div>
                ) : (
                    <p className="text-center text-muted-foreground py-20">You haven't liked any videos yet.</p>
                )}
            </div>
        </div>
    </MainContent>
